...
// --- Brands Data (We only need the names and URLs now!) ---
const brandLogos = [
  { name: "Trillion Club", url: "https://trillionclubsa.com/" },
  { name: "Fluency", url: "https://fluency.live/" }, 
  { name: "Icona", url: "https://icona.ae/" },
  { name: "Reborn", url: "https://rebornegypt.com/" },
  { name: "Torinese", url: "https://www.instagram.com/torinese.eg/" },
  { name: "Roots", url: "https://www.instagram.com/roots_hairrepair/" },
  { name: "Viola", url: "https://www.instagram.com/getviola/" },
  { name: "Naturel", url: "https://www.instagram.com/___naturel___/" },
  { name: "The Rx Hair", url: "https://www.instagram.com/therxhair/" },
  { name: "Robust", url: "https://www.facebook.com/robustsportswear/" }
];
...
// --- NEW ROW 3: HUGE GLOWING TEXT STRIP ---
const BrandsBannerStrip = ({ items, speed = 50 }) => {
  const containerRef = useRef(null);
  const [isPaused, setIsPaused] = useState(false);
  ...
  return (
    <div
      ref={containerRef}
      className="w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] overflow-x-hidden flex items-center py-12 md:py-16"
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
    >
      <div className="flex gap-6 md:gap-12 px-4 items-center">
        {duplicated.map((brand, i) => (
          <a
            ...
            className="group relative flex-shrink-0 flex items-center justify-center 
                       px-12 py-8 md:px-20 md:py-12 
                       bg-gradient-to-br from-neutral-800/90 to-neutral-950/90 
                       border border-neutral-700/50 rounded-3xl 
                       shadow-[0_4px_20px_rgba(0,0,0,0.4)] 
                       hover:shadow-[0_0_50px_rgba(20,184,166,0.5)] 
                       hover:border-teal-400/80 
                       transition-all duration-500 ease-out transform hover:-translate-y-3 hover:scale-105 
                       overflow-hidden"
          >
            {/* Inner Glow Flare on Hover */}
            <div className="absolute inset-0 bg-gradient-to-tr from-teal-500/0 via-teal-500/0 to-teal-500/15 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            
            {/* MASSIVE TEXT */}
            <span className="relative z-10 text-4xl md:text-6xl font-black uppercase tracking-[0.2em] whitespace-nowrap
                             bg-clip-text text-transparent bg-gradient-to-b from-white to-neutral-500 
                             group-hover:from-teal-300 group-hover:to-sky-500 transition-all duration-500">
              {brand.name}
            </span>
          </a>
        ))}
    ...
    >
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-4 pb-24">
        ...
        <SectionWrapper ref={sectionRefs.results} id="results" title="Results">
          ...
          {/* New Massive Glowing Text Strip placed at the bottom of results */}
